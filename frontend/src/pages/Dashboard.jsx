...
const Dashboard = () => {
  ...
  const [deviceCostBreakdown, setDeviceCostBreakdown] = useState([]);
  ...
  useEffect(() => {
    ...
    const loadDeviceCosts = async () => {
      try {
        const costByDevice = await Promise.all(
          devices.map(async (device) => {
            const result = await fetchSeries(device.deviceId, filters.from, filters.to);
            const energy = Number(result?.stats?.totalEnergy || 0);
            ...
            return {
              ...
              isSelected: device.deviceId === filters.deviceId
            };
          })
        );

        if (!isCancelled) {
          setDeviceCostBreakdown(costByDevice.sort((a, b) => b.cost - a.cost));
        }
      } catch (err) {
        console.error("Error loading device cost breakdown:", err);
      }
    };
  ...
  }, [devices, filters.from, filters.to, filters.deviceId]);
  ...
  const kpis = [
    ...
    {
      label: "Cost Estimate",
      value: formatNumber(stats.totalEnergy * COST_PER_KWH, " INR"),
  ...
  const totalEnergy = Number(stats.totalEnergy || 0);
  const totalCost = Number(advancedMetrics?.costAnalysis?.totalCost || totalEnergy * COST_PER_KWH);
  const selectedDeviceName = devices.find((d) => d.deviceId === filters.deviceId)?.name || filters.deviceId;
      ...
      {activeTab === "overview" && (
        <div className="space-y-4 sm:space-y-6">
          ...
          <CostEstimationPanel
            ...
            advancedCost={advancedMetrics?.costAnalysis}
            deviceCostBreakdown={deviceCostBreakdown}
